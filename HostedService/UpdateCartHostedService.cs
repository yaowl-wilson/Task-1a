...
namespace OrderService.HostedServices
{
    ...
    public class UpdateCartHostedService : BackgroundService, IDisposable
    {
        private readonly CartModelContext _cartModelContext;
        ...
        public UpdateCartHostedService(
            ILoggerFactory loggerFactory,
            CartModelContext context)
        {
            _logger = loggerFactory.CreateLogger<UpdateCartHostedService>();
            _cartModelContext = context;
        ...
        private async void HandleMessage(string content)
        {
            ...
            try
            {
                CartModel orderModel = JsonConvert.DeserializeObject<CartModel>(content);
                _cartModelContext.Update(orderModel);

                List<CartModel> cartList = _cartModelContext.CartModelItems.Where(u => u.CartID == orderModel.CartID).ToList();
                cartList.ForEach(a => { a.OrderStatusID = orderModel.OrderStatusID; });
                _cartModelContext.SaveChanges();
            }
            catch(Exception ex)
            {
